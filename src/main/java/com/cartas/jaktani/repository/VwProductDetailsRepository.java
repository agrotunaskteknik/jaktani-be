...
public interface VwProductDetailsRepository extends JpaRepository<VwProductDetails, Integer> {
    ...
    @Query("FROM VwProductDetails vw WHERE vw.shopId =:shopId AND vw.categoryId =:categoryId " +
            "AND vw.shopId <> 14 AND (LOWER(vw.productName) LIKE %:keySearch% OR LOWER(vw.brand) LIKE %:keySearch%) ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllByShopIdAndCategoryIdAndKeySearchMitra(Pageable pageable, @Param("shopId") Integer shopId, @Param("categoryId") Integer categoryId, @Param("keySearch") String keySearch);

    @Query("FROM VwProductDetails vw WHERE vw.shopId =:shopId AND vw.shopId <> 14 " +
            "AND (LOWER(vw.productName) LIKE %:keySearch% OR LOWER(vw.brand) LIKE %:keySearch% OR LOWER(vw.categoryName) LIKE %:keySearch%) ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllByShopIdAndKeySearchMitra(Pageable pageable, @Param("shopId") Integer shopId, @Param("keySearch") String keySearch);

    @Query("FROM VwProductDetails vw WHERE vw.categoryId =:categoryId AND vw.shopId <> 14 " +
            "AND (LOWER(vw.productName) LIKE %:keySearch% OR LOWER(vw.brand) LIKE %:keySearch% OR LOWER(vw.shopName) LIKE %:keySearch%) ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllByCategoryIdAndKeySearchMitra(Pageable pageable, @Param("categoryId") Integer categoryId, @Param("keySearch") String keySearch);

    @Query("FROM VwProductDetails vw WHERE vw.shopId =:shopId AND vw.shopId <> 14 AND vw.categoryId =:categoryId ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllByShopIdAndCategoryIdMitra(Pageable pageable, @Param("shopId") Integer shopId, @Param("categoryId") Integer categoryId);

    @Query("FROM VwProductDetails vw WHERE vw.shopId =:shopId AND vw.shopId <> 14 ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllByShopIdMitra(Pageable pageable, @Param("shopId") Integer shopId);

    @Query("FROM VwProductDetails vw WHERE vw.categoryId =:categoryId AND vw.shopId <> 14 ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllByCategoryIdMitra(Pageable pageable, @Param("categoryId") Integer categoryId);

    @Query("FROM VwProductDetails vw WHERE LOWER(vw.productName) LIKE %:keySearch% " +
            "OR LOWER(vw.categoryName) LIKE %:keySearch% OR LOWER(vw.shopName) LIKE %:keySearch% " +
            "OR LOWER(vw.brand) LIKE %:keySearch% AND vw.shopId <> 14 ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllWithKeySearchMitra(Pageable pageable, @Param("keySearch") String keySearch);

    @Query("FROM VwProductDetails vw WHERE vw.shopId <> 14 ORDER BY vw.createdTime asc")
    List<VwProductDetails> findAllWithPagingMitra(Pageable pageable);
